...
class KiwoomCore:
    ...
    def __init__(
        ...
        acc_id: str = "",
        apis_spec_path: Optional[str] = None,
    ):
        self.appkey = appkey
    ...
    def _check_kiwoom_error(self, resp: requests.Response) -> Dict[str, Any]:
        if not resp.ok:
            raise KiwoomException(
                return_code=f"HTTP_{resp.status_code}", return_msg=resp.text, status_code=resp.status_code
            )
        ...
        if return_code != "0":
            ...
            raise KiwoomException(return_code=return_code, return_msg=return_msg, status_code=resp.status_code)
    ...
    async def connect_ws(self, on_message: Callable[[Dict[str, Any]], Any]):
        ...
        headers = {"authorization": f"Bearer {token}", "appkey": self.appkey, "secretkey": self.secretkey}

        self._ws = await websockets.connect(self.ws_url, additional_headers=headers)

        # [수정] 웹소켓 접속 직후 'LOGIN' 페이로드 전송 (키움 공식 스펙 반영)
        auth_payload = {"trnm": "LOGIN", "token": token}
    ...
    def revoke_token(self) -> Dict[str, Any]:
        ...
        try:
            ...
            resp = requests.post(
                ...
                json={"appkey": self.appkey, "secretkey": self.secretkey, "token": self._token},
    ...
    def request(self, method: str, path: str, headers: Optional[Dict[str, str]] = None, **kwargs) -> Dict[str, Any]:
        ...
        **kwargs를 통해 json, params 등 requests 라이브러리가 지원하는 인자를 유연하게 전달할 수 있습니다.
        ...
        req_headers = {"authorization": f"Bearer {token}", "Content-Type": "application/json;charset=UTF-8"}
        ...
        try:
            ...
            is_token_invalid = (
                str(body_for_check.get("return_code")) == "3"
                or "인증에 실패" in str(body_for_check.get("return_msg", ""))
                or "Token이 유효하지 않습니다" in str(body_for_check.get("return_msg", ""))
            )
            ...
            return {"headers": dict(resp.headers), "body": body}
    ...
    def get(
        self, path: str, headers: Optional[Dict[str, str]] = None, params: Optional[Dict[str, Any]] = None, **kwargs
    ) -> Dict[str, Any]:
        """GET 요청 래퍼"""
        return self.request("GET", path, headers=headers, params=params, **kwargs)

    def post(
        self, path: str, headers: Optional[Dict[str, str]] = None, json_data: Optional[Dict[str, Any]] = None, **kwargs
    ) -> Dict[str, Any]:
        """POST 요청 래퍼"""
