...
for api in apis:
    raw_base_name = clean_base_name(api["class_name"])
    if raw_base_name in seen_base_names:
        seen_base_names[raw_base_name] += 1
        api["clean_base_name"] = f"{raw_base_name}{seen_base_names[raw_base_name]}"
    else:
        seen_base_names[raw_base_name] = 1
        api["clean_base_name"] = raw_base_name
...
# ====================================================================
for api in apis:
    if api.get("id") == "ka50081":
        api["class_name"] = "GoldSpotDailyChart"
    elif api.get("id") == "ka50092":
        api["class_name"] = "GoldSpotMinuteChart"
...
def generate_sub_models(api, items, parent_class_name):
    ...
    for item in items:
        ...
        if item.get("children"):
            ...
            sub_model_classes.append("    model_config = ConfigDict(populate_by_name=True)")
            if not sub_fields:
            ...
            field_declarations.append(
                f'    {py_key}: Annotated[List[{sub_class_name}], BeforeValidator(_force_list)] = Field(default_factory=list{alias_str}, description="{desc}")'
            )
        else:
            ktype = item.get("type", "").replace(" ", "")
            if ktype == "String[]":
                field_declarations.append(
                    f'    {py_key}: SafeListStr = Field(default_factory=list{alias_str}, description="{desc}")'
                )
            elif ktype == "int":
...
for api in apis:
    base_name = api["clean_base_name"]
    req_class_name = f"{base_name}Request"
    # Response라는 단어를 없애고, 순수 데이터 모델 객체명으로 승격
    res_class_name = base_name

    # ---------------- REQUEST MODEL ----------------
    req_sub_classes, req_fields = generate_sub_models(
        api, api.get("headers", []) + api.get("params", []), req_class_name
    )
    ...
    # ---------------- RESPONSE MODEL ----------------
    res_sub_classes, res_fields = generate_sub_models(
        api, api.get("res_headers", []) + api.get("res_body", []), res_class_name
    )
...
for api in apis:
    ...
    for item in items:
        ...
        call_kwargs.append(f'"{real_key}": {py_key}')
    ...
    if api.get("path") == "/api/dostk/websocket":
        ...
        out.append('        """')
        out.append(f"        req = {req_class_name}(**{{{kwargs_str}}})  # type: ignore")
        out.append("        await self.core.send_ws(req.model_dump(by_alias=True, exclude_none=True))")
        out.append("")
    else:
        ...
        out.append(f"        req = {req_class_name}(**{{{kwargs_str}}})  # type: ignore")
        out.append(
            f'        raw_response = self.core.call("{api["id"]}", **req.model_dump(by_alias=True, exclude_none=True))'
        )
